"""Build plate v121 from v121: the day's closed rows, and a tail for the new ones.
...
The new rows the tester filed today (226, 228, 230, 232-236) are not in v121's
...
SRC = "FOR_TORNIKE_MISHO_PLATE_2026-09-21_v121_SRULI.docx"
OUT = "FOR_TORNIKE_MISHO_PLATE_2026-09-21_v122_SRULI.docx"
ID_COLUMN = 1
COMMENT_COLUMN = 6
...
def main(spec_path: str) -> None:
    ...
    if missing:
        raise SystemExit(f"plate_v122: rows not found in the table: {missing}")

    for paragraph in document.paragraphs:
        if paragraph.text.startswith("v121 —"):
            set_paragraph(paragraph, spec["intro"][0])
        elif paragraph.text.startswith("v120-თან შედარებით"):
