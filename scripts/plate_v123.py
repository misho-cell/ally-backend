"""Build plate v123 from v122: the four frontend rows that closed tonight.

Five cells rewritten and two header lines replaced. Everything else is copied,
so a row nobody worked on keeps the words it had.

93, 218 and 91 closed on the frontend's own builds tonight; 88 stays amber
because their fix is right and my measurement says it cannot fire on this
data; 111 becomes "waiting for evidence from the phone" rather than "not
started", which is a different sentence and the honest one.
"""
...
SRC = "FOR_TORNIKE_MISHO_PLATE_2026-09-21_v122_SRULI.docx"
OUT = "FOR_TORNIKE_MISHO_PLATE_2026-09-21_v123_SRULI.docx"
ID_COLUMN = 1
COMMENT_COLUMN = 6
...
def main(spec_path: str) -> None:
    ...
    if missing:
        raise SystemExit(f"plate_v123: rows not found in the table: {missing}")

    for paragraph in document.paragraphs:
        if paragraph.text.startswith("v122 —"):
            set_paragraph(paragraph, spec["intro"][0])
        elif paragraph.text.startswith("v121-თან შედარებით"):
